...
class TestcaseRunningTest(fake_filesystem_unittest.TestCase):
  ...
  def setUp(self):
    ...
    os.environ['BOT_TMPDIR'] = '/bot_working_directory/tmp'
    os.environ['TEST_TMPDIR'] = '/bot_working_directory/tmp'
    ...
    fuzztarget = data_types.FuzzTarget(fuzzer_engine='engine', binary='target', project="test_project")
    data_handler.add_fuzz_target(fuzztarget)

    self.blackbox_testcase = data_types.Testcase(test_case=b'', fixed='False', one_time_crasher_flag=False, comments="", absolute_path="",
                                                 queue="", archived=False, timestamp=datetime.datetime.now(), triaged=False, has_bug_flag=False, open=False,
                                                 testcase_path="", minimized_keys="", minidump_keys="", fuzzed_keys="",
                                                 additional_metadata="", minimized_arguments="")
    self.greybox_testcase = data_types.Testcase(test_case=b'', fixed='False', one_time_crasher_flag=False, comments="", absolute_path="",
                                                 queue="", archived=False, timestamp=datetime.datetime.now(), triaged=False, has_bug_flag=False, open=False,
                                                 testcase_path="", minimized_keys="", minidump_keys="", fuzzed_keys="",
                                                 additional_metadata="", minimized_arguments="")
    ...
    self.fs.create_dir('/bot_working_directory/tmp')
...
# "Test that we can reproduce
# a testcase with libFuzzer."
#
# The first thing to notice is that the class inherits from
# `untrusted_runner_helpers.UntrustedRunnerIntegrationTest`. This is a base class
# that sets up the environment for running tests in the untrusted environment
