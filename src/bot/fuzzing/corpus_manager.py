...
class FuzzTargetCorpus(CorpusStorage):
    ...
    Returns:
      A bool indicating whether or not the command succeeded.
    """
        result = CorpusStorage.rsync_from_disk(
    ...
    Returns:
      A bool indicating whether or not the command succeeded.
    """
        result = CorpusStorage.rsync_to_disk(
            self, directory, timeout=timeout, delete=delete)
        if not result:
            return False
    ...
    def get_regressions_corpus_gcs_url(self):
        ...
        return self.get_storage_path(suffix=REGRESSIONS_GCS_PATH_SUFFIX)
