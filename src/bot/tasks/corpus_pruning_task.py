...
class Context(object):
    ...
    def __init__(self,
        ...
        self.shared_corpus = corpus_manager.CorpusStorage(shared_corpus_bucket)
...
def do_corpus_pruning(context, last_execution_failed, revision):
    ...
    if last_execution_failed:
        for corpus_url in [
            context.corpus.get_storage_path(),
            context.quarantine_corpus.get_storage_path()
        ]:
    ...
    coverage_info.corpus_location = context.corpus.get_storage_path()
    coverage_info.quarantine_location = context.quarantine_corpus.get_storage_path()
...
def _process_corpus_crashes(context, result):
    ...
    for crash in result.crashes:
        ...
        with open(unit_path, 'rb') as f:
           key = blobs_manager.write_blob(f)
...
def execute_task(full_fuzzer_name, job_type):
    ...
    fuzz_target = data_handler.get_fuzz_target_by_id(full_fuzzer_name)
    task_name = 'corpus_pruning_%s_%s' % (full_fuzzer_name, job_type)
    revision = 0  # Trunk revision

    # Get status of last execution.
    bot_name = environment.get_value('BOT_NAME')
    last_execution_metadata = data_handler.get_task_status(bot_name, task_name)
