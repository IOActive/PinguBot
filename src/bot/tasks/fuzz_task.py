...
def find_main_crash(crashes, fuzzer_name, fuzztarget_id, test_timeout):
...
class CrashGroup(object):
    ...
    def __init__(self, crashes, context):
        ...
        self.main_crash, self.one_time_crasher_flag = find_main_crash(
            crashes=crashes, fuzzer_name=context.fuzzer_name, fuzztarget_id=context.fuzz_target.id,
            test_timeout=context.test_timeout)
...
class SyncCorpusStorage(object):
    ...
    def sync_from_storage(self):
        ...
        if last_sync_time and os.path.exists(self._corpus_directory):
            last_update_time = storage.last_updated(self.corpus_storage.get_storage_path())
        ...
        result = already_synced or self.corpus_storage.rsync_to_disk(
...
def create_testcase(group, context):
    ...
    store_crash(crash_obj=crash, job_type=context.job_type, testcase_id=testcase_id,
                regression=get_regression(group.one_time_crasher_flag), crash_revision=context.crash_revision,
                reproducible_flag=group.one_time_crasher_flag, iteration=0, one_time_crasher_flag=group.one_time_crasher_flag)
...
def get_engine(context):
    ...
    if context.fuzz_target:
        return context.fuzz_target.fuzzer_engine
...
def _update_testcase_variant_if_needed(group, context):
  ...
  variant on this job."""
    ...
    variant = data_handler.get_testcase_variant(str(group.existing_testcase.id),
                                                context.job_type)
    ...
    data_handler.update_testcase_variant(variant)
...
class FuzzingSession(object):
    ...
    def sync_corpus(self, sync_corpus_directory):
        ...
        self.corpus_storage = SyncCorpusStorage(self.fuzzer_name,
    ...
    def _save_fuzz_targets_count(self):
        ...
        targets_count = data_types.FuzzTargetsCount
    ...
    def do_engine_fuzzing(self, engine_impl):
        ...
        # Synchronize corpus files with Minio
    ...
    def run(self):
        ...
        engine_impl = engine.PythonFuzzerEngine.get(self.fuzzer.name)
