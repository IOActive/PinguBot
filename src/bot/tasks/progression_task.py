...
def _check_fixed_for_custom_binary(testcase, job_type, testcase_file_path):
    ...
    result = testcase_manager.test_for_crash_with_retries(
        testcase, testcase_file_path, test_timeout, crash, http_flag=testcase.http_flag)
...
def _store_testcase_for_regression_testing(testcase, testcase_file_path):
  ...
  pruning task."""
    ...
    fuzz_target = data_handler.get_fuzz_target_by_id(testcase.overridden_fuzzer_name)
    if not fuzz_target:
...
def find_fixed_range(testcase_id, job_type):
    ...
    result = _testcase_reproduces_in_revision(testcase, testcase_file_path,
                                              job_type, min_revision, crash=crash)
    ...
    while time.time() < deadline:
        ...
        try:
            result = _testcase_reproduces_in_revision(testcase, testcase_file_path,
                                                      job_type, middle_revision, crash=crash)
