...
class AflRunnerCommon:
  ...
  DUMP_EXTERNAL_UTILITY_REGEX = re.compile(r'AFL_I_DONT_CARE_ABOUT_MISSING_CRASHES=1')
  SKIP_CPUFREQ_REGEX = re.compile(r'AFL_SKIP_CPUFREQ')
...
#                        constants.INPUT_BINARY)
  ...
  def prepare_retry_if_skip_cpu_freq(self, fuzz_result):
    """
    Whoops, your system uses on-demand CPU frequency scaling, adjusted
    between 390 and 4492 MHz. Unfortunately, the scaling algorithm in the
    ...
    performance drop.
    """
    if re.search(self.SKIP_CPUFREQ_REGEX, fuzz_result.output) is None:
      return False
    ...
    logs.log_error(
        'CPU Frequence error encountered by afl-fuzz\n'
        'Check bot: %s for zombies\n'
        'Trying again with AFL_SKIP_CPUFREQ=1' % BOT_NAME,
        afl_output=fuzz_result.output)
    environment.set_value(constants.AFL_SKIP_CPUFREQ, 1)
    return True
    
    
  def prepare_retry_if_core_dump_to_external_utility(self, fuzz_result):
    """
    ...
    """
    if re.search(self.DUMP_EXTERNAL_UTILITY_REGEX, fuzz_result.output) is None:
      return False
    ...
    # and then try fuzzing again, this time telling AFL not to bind.
    logs.log_error(
        'CPU Core Dump to External utility error encountered by afl-fuzz\n'
        'Check bot: %s for zombies\n'
        'Trying again with AFL_I_DONT_CARE_ABOUT_MISSING_CRASHES=1' % BOT_NAME,
        afl_output=fuzz_result.output)
...
class AflAndroidRunner(AflRunnerCommon, new_process.UnicodeProcessRunner):
  ...
  def fuzz(self):
    ...
    self._fuzz_args = self.generate_afl_args(
        ...
        additional_args=[
            constants.FUZZING_TIMEOUT_FLAG + str(on_device_fuzzing_timeout)
        ],
    ...
    device_script_path = os.path.join(android.constants.DEVICE_FUZZING_DIR,
                                      "run.sh")
    ...
    self._copy_directories_from_device(
        [self.afl_output.output_directory, self._showmap_results_dir])
