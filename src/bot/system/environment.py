...
def get_resources_directory():
    ...
    return os.path.join(get_root_directory(), 'src/bot/resources')
...
def get_value(config_variable, default_value=None):
    """Return an environment variable value."""
    value_string = os.getenv(config_variable)
    ...
    if config_variable == 'ANDROID_SERIAL':
        return value_string
...
def is_engine_fuzzer_job(fuzzer_name=None):
    """Return true if this is an engine fuzzer."""
    return bool(get_engine_for_job(fuzzer_name))


def get_engine_for_job(fuzzer_name=None):
    """Get the engine for the given job."""
    if not fuzzer_name:
        fuzzer_name = get_value('FUZZER_NAME')

    for engine in fuzzing.ENGINES:
        if engine.lower() in fuzzer_name.lower():
            return engine
...
def load_cfg(config_file="bot.cfg"):
    config = configparser.ConfigParser()
    config.read(config_file)
    for variable, value in config.defaults().items():
        if variable in os.environ:
            # Don't override existing values.
            continue
        os.environ[variable.upper()] = str(value)


def set_default_vars():
    """Set default environment vars and values."""
    env_file_path = os.path.join(get_value('ROOT_DIR'), 'bot_working_directory', 'env.yaml')
...
def set_bot_environment():
    ...
    bot_dir = os.path.join(root_dir, 'bot_working_directory')
    ...
    crash_stacks_dir = os.path.join(inputs_dir, 'crash-stacks')
    os.environ['CRASH_STACKTRACES_DIR'] = os.path.join(inputs_dir, crash_stacks_dir)
    os.makedirs(crash_stacks_dir, exist_ok=True)

    data_bundles_dir = os.path.join(inputs_dir, 'data-bundles')
    os.environ['DATA_BUNDLES_DIR'] = data_bundles_dir
    os.makedirs(data_bundles_dir, exist_ok=True)

    fuzzer_testcases_dir = os.path.join(inputs_dir, 'fuzzer-testcases')
    os.environ['FUZZ_INPUTS'] = fuzzer_testcases_dir
    os.makedirs(fuzzer_testcases_dir, exist_ok=True)

    os.environ['FUZZ_INPUTS_MEMORY'] = os.environ['FUZZ_INPUTS']

    fuzzer_testcases_disk_dir = os.path.join(inputs_dir, 'fuzzer-testcases-disk')
    os.environ['FUZZ_INPUTS_DISK'] = fuzzer_testcases_disk_dir
    os.makedirs(fuzzer_testcases_disk_dir, exist_ok=True)

    mutator_plugins_dir = os.path.join(root_dir, 'mutator-plugins')
    os.environ['MUTATOR_PLUGINS_DIR'] = os.path.join(inputs_dir,
                                                     mutator_plugins_dir)
    os.makedirs(mutator_plugins_dir, exist_ok=True)

    fuzz_data_dir = os.path.join(root_dir, 'fuzzer-common-data-bundles')
    os.environ['FUZZ_DATA'] = os.path.join(inputs_dir,
                                           fuzz_data_dir)
    os.makedirs(fuzz_data_dir, exist_ok=True)

    images_dir = os.path.join(inputs_dir, 'images')
    os.environ['IMAGES_DIR'] = images_dir
    os.makedirs(fuzz_data_dir, exist_ok=True)

    symbols_dir = os.path.join(inputs_dir, 'symbols')
    os.environ['SYMBOLS_DIR'] = symbols_dir
