...
def _set_random_fuzz_target_for_fuzzing_if_needed(fuzz_targets, target_weights):
    ...
    if not environment.is_engine_fuzzer_job(environment.get_value("FUZZER_NAME")):
        return None
...
def _set_random_build_scripts_for_fuzzing_if_needed(build_scripts, target_weights):
    """Sets a random fuzz target for fuzzing."""
...
class Build(BaseBuild):
    ...
    def _pick_build_script(self, build_scripts, target_weights):
        """Selects a fuzz target for fuzzing."""
        return _set_random_build_scripts_for_fuzzing_if_needed(
...
def _get_targets_list(bucket_path):
  ...
  fuzz targets."""
    bucket_dir_path = bucket_path  # os.path.dirname(os.path.dirname(bucket_path))
...
def setup_custom_binary(target_weights=None):
    ...
    job = data_handler.get_job(job_name)
