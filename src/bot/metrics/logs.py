...
def get_logging_config_dict(name):
    ...
    logging_handler = {
        'run_bot': get_handler_config('bot_working_directory/logs/bot.log', 3),
        'run': get_handler_config('bot_working_directory/logs/run.log', 1),
        'run_heartbeat': get_handler_config('bot_working_directory/logs/run_heartbeat.log', 1),
        'heartbeat': get_handler_config('bot_working_directory/logs/heartbeat.log', 1),
        'run_fuzzer': get_handler_config('bot_working_directory/logs/run_fuzzer.log', 1),
        'run_testcase': get_handler_config('bot_working_directory/logs/run_testcase.log', 1),
    }
...
def format_record(record: logging.LogRecord) -> str:
    ...
    entry = {'message': truncate(record.getMessage(), STACKDRIVER_LOG_MESSAGE_LIMIT), 'created': (
            datetime.datetime.utcfromtimestamp(record.created).isoformat() + 'Z'), 'severity': record.levelname,
             'bot_name': os.getenv('BOT_NAME'), 'task_payload': os.getenv('TASK_PAYLOAD'), 'name': record.name,
             'location': getattr(record, 'location', {'error': True}), 'extras': getattr(record, 'extras', {})}
